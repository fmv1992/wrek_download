...
def parse_cli_arguments():
    ...
    parser.add_argument(
        "--outputfolder",
        help="output folder to put downloaded files when " "finished.",
        required=True,
    )
    parser.add_argument(
        "--whitelist",
    ...
    for one_folder in LIST_OF_ARGUMENTS_FOLDERS:
        if not os.path.isdir(one_folder):
            raise FileNotFoundError(
                "One of the arguments: '{0}' does " "not exist.".format(
                    one_folder
                )
            )
    if not os.path.isfile(WHITELIST_FILE):
        raise FileNotFoundError(
            "Whitelist file '{0}' does not" "exist.".format(WHITELIST_FILE)
        )

    # Constants
    URL_WREK = "https://www.wrek.org/schedule/"
    URL_M3U = "https://www.wrek.org/playlist.php/main/128kbs/current/"
...
def filter_whitelisted_shows(constants, all_wrek_shows):
    ...
    whitelisted_wrek_shows = sorted(
        ...
        key=lambda x: (
            constants["WEEKDAYS"].index(x.weekday) - dt.now().weekday() - 1
        )
        % 7,
    )
    logging.debug(
...
def main(constants, all_wrek_shows, filtered_wrek_shows):
    ...
    # Show new programs
    set_of_new_programs = set(all_wrek_shows_names) - set(
        auxf.shows_in_whitelist(constants["WHITELIST_FILE"])
    )
    if set_of_new_programs:
        logging.info(
            "New shows not included in the whitelist:\n%s",
        ...
        if constants["BATCH_MODE"]:
            auxf.include_programs_in_whitelist(
                constants["WHITELIST_FILE"], sorted(set_of_new_programs)
            )
        else:
            if (
                input(
                    "Do you want to include the new programs in your "
                    "whitelist file? [y/n]\n"
                )
                == "y"
            ):
    ...
    while (
        download_queue.unfinished_tasks
        and time.time() < stop
    ...
    if not exception_queue.qsize() < constants["N_THREADS"]:
        logging.debug(
            "All threads ({0}) have died. Exit code: 1.".format(
