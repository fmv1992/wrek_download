...
def update_m3u_files(constants, filtered_wrek_shows):
    ...
    remote_m3u_file_names = re.findall(
        '(?<=current\/)[^>]*?\.m3u(?=">128k<)', text
    )
    ...
    for one_show in [
        ...
        if show.m3u_filename in remote_m3u_file_names
    ]:
    ...
    remote_m3u_content = {
        k: re.sub("(?<!_old)\.mp3", "_old.mp3", v)
        for k, v in remote_m3u_content.items()
    }
